...
@section('title', 'Laporan Keuangan Klinik')
@section('dashboard-title', 'Laporan Keuangan Klinik')
...
<div class="space-y-6">
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form method="GET" action="{{ route('kasir-klinik.laporan') }}" class="flex flex-wrap items-end gap-4">
            <div class="flex-1 min-w-48">
                <label class="block text-sm font-medium text-gray-700 mb-2">Pilih Periode</label>
                <select name="period" id="period_filter" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-pink-500">
            ...
            <div class="flex-1 min-w-48 {{ $period != 'custom' ? 'hidden' : '' }}" id="custom_start_wrapper">
                ...
                <input type="date" name="custom_start" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-pink-500" value="{{ $startDate->format('Y-m-d') }}">
            </div>
            
            <div class="flex-1 min-w-48 {{ $period != 'custom' ? 'hidden' : '' }}" id="custom_end_wrapper">
                <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Akhir</label>
                <input type="date" name="custom_end" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-pink-500" value="{{ $endDate->format('Y-m-d') }}">
            </div>

            <button type="submit" class="bg-pink-600 text-white px-6 py-2 rounded-md hover:bg-pink-700 transition-colors font-medium">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="p-6 border-b border-gray-200 flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-800">
                Laporan Keuangan Klinik ({{ $startDate->format('d/m/Y') }} - {{ $endDate->format('d/m/Y') }})
            </h3>
        ...
        <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-pink-50 p-4 rounded-lg border-l-4 border-pink-600">
                    ...
                    <p class="text-2xl font-bold text-pink-600 mt-1">Rp {{ number_format($laporan['total_revenue'], 0, ',', '.') }}</p>
                </div>
            ...
            <div class="border p-4 rounded-lg">
                <h4 class="font-semibold text-gray-800 mb-3 border-b pb-2">Pendapatan Berdasarkan Jenis Layanan</h4>
                <div class="grid grid-cols-2 gap-4">
                    @foreach($laporan['by_service_type'] as $type => $data)
                    <div class="p-3 bg-gray-50 rounded-lg">
                        <p class="text-sm text-gray-600">{{ ucfirst($type) }} ({{ $data['count'] }} Transaksi)</p>
            ...
            <div class="border p-4 rounded-lg">
                <h4 class="font-semibold text-gray-800 mb-3 border-b pb-2">Pendapatan Berdasarkan Metode Pembayaran</h4>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">Total Pendapatan</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2 whitespace-nowrap text-right font-bold text-sm text-pink-600">Rp {{ number_format($data['total'], 0, ',', '.') }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            <div class="border p-4 rounded-lg">
                <h4 class="font-semibold text-gray-800 mb-3 border-b pb-2">Detail Setiap Transaksi</h4>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Waktu</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">ID Tagihan</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 text-sm">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2 whitespace-nowrap font-medium text-pink-600">#{{ $transaction->tagihan_id }}</td>
                                <td class="px-4 py-2 text-gray-900">{{ $transaction->tagihan->pasien->nama ?? 'N/A' }}</td>
                                <td class="px-4 py-2 whitespace-nowrap text-gray-700">{{ ucfirst($transaction->tagihan->jenis_tagihan) }}</td>
                                <td class="px-4 py-2 whitespace-nowrap text-right font-bold text-pink-600">Rp {{ number_format($transaction->jumlah_bayar, 0, ',', '.') }}</td>
