...
@section('title', 'Tambah Staf | Admin Ganesha Hospital')
@section('dashboard-title', 'Tambah Staf')
...
<div class="mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-xl font-bold text-gray-800">Tambah Staf Baru</h2>
                <p class="text-sm text-gray-600 mt-1">Lengkapi form di bawah untuk menambahkan staf</p>
            </div>
        </div>

        <form action="{{ route('admin.staf.store') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="bagian" id="bagian" required
                            ...
                            <option value="pendaftaran" {{ old('bagian') === 'pendaftaran' ? 'selected' : '' }}>Pendaftaran</option>
                            <option value="farmasi" {{ old('bagian') === 'farmasi' ? 'selected' : '' }}>Farmasi</option>
                            <option value="laboratorium" {{ old('bagian') === 'laboratorium' ? 'selected' : '' }}>Laboratorium</option>
                            <option value="kasir" {{ old('bagian') === 'kasir' ? 'selected' : '' }}>Kasir</option>
                        </select>
                        @error('bagian')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
            ...
            <div class="flex items-center justify-end gap-4 pt-4">
                <a href="{{ route('admin.staf.index') }}" class="px-6 py-2 bg-white text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                    Batal
                </a>
